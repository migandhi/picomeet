/* PicoMeet mesh engine — one RTCPeerConnection per remote peer.
   Implements the W3C "perfect negotiation" pattern so glare never deadlocks. */
export class Mesh extends EventTarget {
  constructor({ selfId, send, ice }) {
    ...
    this.ice = ice;
    this.peers = new Map();           // id -> {pc, dc, polite, making, ignore, srdAnswerPending, stream}
    this.local = null;                // MediaStream currently published
  ...
  /* ---------------------------------------------------------------- peers */
    ...
    const p = {
      ...
      making: false, ignore: false, srdAnswerPending: false,
      stream: new MediaStream()
    };
    this.peers.set(id, p);
    // One reliable, ordered data channel per peer: ink, files, cursors.
    if (!p.polite) {                 // impolite side creates it; polite side listens
  ...
  async onSignal(from, data) {
    ...
    try {
      if (data.desc) {
        ...
        catch (err) { if (!p.ignore) throw err; }
      }
  ...
  /** Swap the outgoing video track everywhere (camera <-> screen) with zero renegotiation. */
  async replaceVideo(track) {
    for (const p of this.peers.values()) {
      ...
      else if (track) p.pc.addTrack(track, this.local);
      this.applyQuality(p);
    }
  }
  /** Enforce the Adaptive Mesh Governor contract on every outbound stream. */
  setQuality(q) { this.quality = { ...this.quality, ...q }; this.peers.forEach(p => this.applyQuality(p)); }
  ...
  destroy() { [...this.peers.keys()].forEach(id => this.close(id)); }
  /** Outbound health probe used by quality.js */
  async stats() {
    const out = [];
    for (const p of this.peers.values()) {
